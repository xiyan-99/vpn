...
A: 一个模块实例只能监控一个源。如果要监控多个源，需要安装多个模块实例（复制并修改模块文件）。
...
1. 复制模块文件：
```bash
cydia-repo-monitor-chariz.sgmodule
cydia-repo-monitor-packix.sgmodule
cydia-repo-monitor-havoc.sgmodule
```

2. 修改每个文件的内容：
```ini
#!name=Cydia源监控-Chariz
[Panel]
cydia_chariz = script-name=cydia_chariz,update-interval=7200
[Script]
cydia_chariz = type=generic,...
```
