...
**注意事项：**
- Cron 任务会在后台运行，有变更时会发送通知
- 如果不需要定时任务，可以在 Surge 模块管理中禁用 `cydia_repo_monitor_cron` 脚本
...
MAXSHOW = 10
```
**说明：** 每4小时自动检查一次，有变更时发送通知

### 示例3：监控多个源（一行一个，每天固定时间检查）
...
1. 找到本模块
2. 点击展开脚本列表
3. 禁用 `cydia_repo_monitor_cron` 脚本

**方法2：** 不推荐留空 CRON 参数（会导致错误），请使用方法1
